   ...
   display: none;
    ...
    <div class="for-content-tabs">
        ...
        {{ Session::get('alert-class', 'alert-success') }}">{{ Session::get('message') }}
            ...
            $rating_avg = [];
            @endphp
                @foreach ($getEvaluatorStudents as $key => $value)
                <div class="col-lg-4">
                        <div class="main-tabs-card">
                          ...
                          <div class="tabs-card-name">
                                ...
                                array_push($id_arr,$value->id);
                                 $rating_count++
                                @endphp
                                @foreach ($value->get_rating_by_evualator_for_Studentlist as $items)
                                {{ array_push($rating_avg,$items->rating_start)}}
                                @endforeach
                                </div>
                                <div class="d-flex justify-content-center">
                                    <div class="card-profile-img">
                                        @if ($value->get_bookedStudentByEvaluator->image ?? '')
                                        <img src="{{ asset('storage/uploads/cms/' . $value->get_bookedStudentByEvaluator->image ??'') }}" alt="image">
                                        @else
                                        <img src="{{ (!empty($Value->get_bookedStudentByEvaluator->image ??''))?
                                            asset('storage/uploads/cms/'.$Value->get_bookedStudentByEvaluator->image ??''):asset('storage/uploads/No-image.jpg') }}"
                                          >
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="tabs-card-name">
                                    <h1>{{$value->get_bookedStudentByEvaluator->name ?? ''}}</h1>
                                    <div class="tabs-card-name">
                                    {{-- @if (!empty($value->get_rating_by_evualator_for__studentlist_avg_rating_start ?? ''))
                                                <div class="tabs-card-name">
                                                    <div class="rricons">
                                                        @php
                                                    $conditioned_stars = ceil($value->get_rating_by_evualator_for__studentlist_avg_rating_start) > 5 ? 5 : ceil($value->get_rating_by_evualator_for__studentlist_avg_rating_start);
                            ...
                            <div id="divMain">
                                <div id="rating_show_on_profile_Evaluator{{ $value->id }}"></div>
                            </div>

                                <div class="tabs-card-name">
                                    <p><span>{{@$value->get_bookedStudentByEvaluator->email ?? ''}}</span></p>
                                </div>
                                <div class="tabs-card-name">
                                    <p><span>{{@$value->get_bookedStudentByEvaluator->contact ?? ''}}</span></p>
                                </div>
                                ...
                                @if ($value->get_rating_by_evualator_for__studentlist_avg_rating_start)
                                <div class="tabs-inner-button">
                                    <button class="btn for-complete-cstm-btn" data-bs-toggle="modal" data-bs-target="#CompleteModalBtn"  onclick="get_user_id({{ $value->get_bookedStudentByEvaluator->id }},{{ $value->id }})" disabled>Completed</button>
                                </div>
                                @else
                                <div class="tabs-inner-button">
                                    <button class="btn btn-default" data-bs-toggle="modal" data-bs-target="#CompleteModalBtn"  onclick="get_user_id({{ $value->get_bookedStudentByEvaluator->id }},{{ $value->id }})">Complete</button>
                                </div>
                                @endif
                                <div class="tabs-inner-button">
                                    <a href="{{ route('assesment_create',$value->id) }}"><button class="btn btn-default" data-bs-toggle="modal">Assesment</button></a>
                                </div>
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                {{-- <form id="CompleteModal">
                                    {{ csrf_field() }} --}}
                                    <form action="{{ route('StudentRateing',$value->get_bookedStudentByEvaluator->id ?? '') }}"  enctype="multipart/form-data" method="post">
...
<script>
    ...
    $(document).ready(function () {
        ...
        for(var x = 0; x < rating_item; x++){
            ...
            $('#rating_show_on_profile_Evaluator'+json_arr[x]).jsRapStar({
            value: get_ratings[x],
            colorFront: 'gold',
            enabled: false,
            step: false,
