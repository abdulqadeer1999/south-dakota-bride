...
@section('content')
    ...
    <div class="for-content-tabs">
        ...
        {{ Session::get('alert-class', 'alert-success') }}">{{ Session::get('message') }}
            ...
            <!-- card started -->
            @foreach ($getCoache_profile as $key => $value)
            <div class="profile-user-inner">
                ...
                <form   action="{{ route('CoachBooking.create',$value->id) }}" enctype="multipart/form-data"
                method="post">
                @csrf
                <div class="row mt2">
                    <div class="col-lg-6">
                    ...
                    <div class="col-lg-6">
                        <div class="user-information">
                            <label for="cars">Time Slot:</label>

                            <select name="time_slot" id="time_slot_id">
                                <option disabled value="1">No Time Slot Available</option>
                                @foreach ( $getAppoinment as  $Appoinment )
                                <option value=" {{ $Appoinment->id}}">
                                   {{ $Appoinment->appoinment_date}} -- {{ $Appoinment->appoinment_time}}
                                </option>
                                @endforeach
                                @error('time_slot')
                ...
                <div class="for-btns-width">
                    <div class="tabs-inner-button mt1">
                        {{-- {{ route('') }} --}}
                        <button type="submit" class="black-btn" id="BookingModal">Book</button>
            ...
            <div class="modal cstm-modal" id="messageModalBtn" tabindex="-1" aria-labelledby="messageModalBtnLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title coachName" id="messageModalBtnLabel">Message {{ $value->name }}
                        </h5>
...
$(document).ready(function (){
    ...
    var time_slot = $('#time_slot_id').val();
    // console.log(time_slot);
    if(time_slot == null){
        toastr.error('No Time Slot Available for this Coach', 'Dear User');
        return false;
    }
    });
});
